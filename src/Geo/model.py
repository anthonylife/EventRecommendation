...
def kernelInfluence(mean, dis_variance, x):
    dis = np.sqrt(np.sum((x-mean)**2))
    return 1./dis_variance*normGaussian(dis/dis_variance)
...
def normalize(var):
    ...
    if norm == 0:
        ...
        raw_input()
        globals()['num_zero'] += 1
        var = [0.0 for i in xrange(len(var))]
    else:
        var = var/np.sum(var)
    return var
...
class GeoMF_D():
    def __init__(self, cluster_method=2, cluter_tag=False, train_path=None, event_info_path=None, city_id=None):
        ...
        if cluter_tag == True:
            ...
            cluster_fd = open(settings["GEOMF-D_CLUSTER"], "wb")
            self.cluster_centres = pickle.load(cluster_fd)
    ...
    def model_init(self, train_file, eventinfo_file):
        ...
        self.user_geo_factor = np.array([rPosGaussian(self.n_components) for i in
            xrange(len(self.user_ids))])
        self.event_geo_factor = np.array(self.calGeoInfluence(means, variances, self.event_geo, 1))
        #self.event_geo_factor = np.array(self.calGeoInfluence(means, dis_variances, self.event_geo, 2))
        ...
        for i, pair in enumerate(self.tr_pairs):
            ...
            for j in xrange(scan_idx, len(self.pool_eids)):
                ...
                if neg_eid not in self.user_interacted_event[uid]:
                    self.sgd_update(uid, eid, 0, self.beta1)
                    finished_neg_num += 1
    ...
    def genRecommendResult(self, test_file, result_path):
        ...
        for uid in xrange(len(self.user_ids)):
            ...
            newevent_pref = [[i+self.old_event_num, 0.0] for i in xrange(self.old_event_num, len(self.event_ids))]
            for eid in xrange(self.old_event_num, len(self.event_ids)):
                newevent_pref[eid-self.old_event_num][1] = np.dot(self.user_geo_factor[uid], self.event_geo_factor[eid])
    ...
    def evaluation(self):
        ...
        for uid in xrange(len(self.user_ids)):
            for eid in xrange(self.old_event_num):
                ...
                for i in xrange(self.n_components):
                    pred += np.dot(self.user_geo_factor[uid], self.grid_geo_factor[i])*self.event_grid_dis[eid][i]
                total_error += self.ins_weight[self.user_pref[uid][eid]]*(pred-self.user_pref[uid][eid])**2
            sys.stdout.write("\rFINISHED USER NUM: %d. " % (uid+1))
            sys.stdout.flush()
        error = total_error/(len(self.user_ids)*self.old_event_num)
        print 'Error: %f' % error
    ...
    def genRecommendResult(self, test_file, result_path):
        ...
        for uid in xrange(len(self.user_ids)):
            ...
            for eid in xrange(self.old_event_num, len(self.event_ids)):
                ...
                for i in xrange(self.n_components):
                    pred += np.dot(self.user_geo_factor[uid], self.grid_geo_factor[i])*self.event_grid_dis[eid][i]
                newevent_pref[eid-self.old_event_num][1] = pred
            results = sorted(newevent_pref, key=lambda x:x[1], reverse=True)
            recommendations = [x[0] for x in results]
            for event in recommendations[:settings["RE_TOPK"]]:
                wfd.write(",%s" % event)
